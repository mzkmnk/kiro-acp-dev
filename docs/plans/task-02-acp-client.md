...
- [x] `kiro-cli acp` の子プロセス spawn
  - `child_process.spawn(cliPath, ["acp"], { stdio: ["pipe", "pipe", "pipe"] })`
  - stdin/stdout を JSON-RPC 通信に使用、stderr はログ出力
- [x] `start()` — プロセス起動 + `initialize` リクエスト送信・レスポンス待機
- [x] `stop()` — SIGTERM 送信 → 一定時間後に SIGKILL で確実に終了
- [x] プロセスクラッシュ時の自動再起動（最大3回、カウンタ管理）
- [x] stderr の出力を VSCode OutputChannel にログ出力

### JSON-RPC 2.0 通信

- [x] 送信: JSON をシリアライズして stdin に書き込み（NDJSON: 1行1メッセージ + `\n`）
- [x] 受信: stdout からのデータをバッファリングし、改行区切りで完全なメッセージ単位にパース
- [x] リクエスト ID: インクリメンタルな整数（`private nextId = 0`）
- [x] 未解決リクエスト管理: `Map<number, { resolve, reject }>` でレスポンス受信時に Promise を resolve
- [x] タイムアウト: 一定時間レスポンスがない場合に reject

### 受信メッセージの振り分け

- [x] `id` あり + `result`/`error` あり → レスポンス → 対応する Promise を resolve/reject
- [x] `id` あり + `method` あり → Agent → Client リクエスト → ハンドラにディスパッチ
- [x] `id` なし + `method` あり → Notification → イベントハンドラに通知
  - `session/update` → `onUpdate` コールバック呼び出し

### 公開メソッド

- [x] `start(): Promise<InitializeResult>` — プロセス起動 + initialize
- [x] `stop(): Promise<void>` — プロセス終了
- [x] `newSession(cwd: string): Promise<SessionNewResult>` — `session/new` 送信
- [x] `prompt(sessionId: string, content: PromptContent[]): Promise<void>` — `session/prompt` 送信
- [x] `cancel(sessionId: string): void` — `session/cancel` Notification 送信（レスポンスなし）
- [x] `onUpdate(callback: (params: SessionUpdateParams) => void): Disposable` — session/update 購読
- [x] `registerHandler(method: string, handler: Function): void` — Agent → Client リクエストのハンドラ登録基盤
